...
export const HAND_FONT = String.raw`
  ...
  function penStroke(pts, size, weight, col, alpha) {
    ...
    for (let pass = 0; pass < passes; pass++) {
      ...
      for (let i = 0; i < pts.length - 1; i++) {
        ...
        const steps = p.max(2, p.ceil(p.dist(ax, ay, bx, by) / 2.2));
        for (let s = 0; s < steps; s++) {
          ...
          p.line(
            p.lerp(ax, bx, t0) + p.random(-0.55, 0.55) * wob,
            p.lerp(ay, by, t0) + p.random(-0.55, 0.55) * wob,
            p.lerp(ax, bx, t1) + p.random(-0.55, 0.55) * wob,
            p.lerp(ay, by, t1) + p.random(-0.55, 0.55) * wob,
          );
        }
      }
  ...
  function handText(str, x, y, size, col, alpha, weight) {
    ...
    const slant = p.random(-0.012, 0.012);
    const drift = p.random(-0.5, 0.5);
    let cx = x;
    for (let i = 0; i < str.length; i++) {
      ...
      if (strokes) {
        p.push();
        p.translate(cx, y + drift * i * 0.05 + p.random(-0.7, 0.7));
        p.rotate(slant + p.random(-0.022, 0.022));
        for (const s of strokes) penStroke(s, size, w, col, alpha);
        p.pop();
      }
      cx += size * advance(ch) + p.random(-0.5, 1.4);
    }
    return cx - x;
  }
`.trim();
